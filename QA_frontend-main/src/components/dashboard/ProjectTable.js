...
const ProjectTables = () => {
  ...
  const getallUsers = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      if (!forceRefresh) {
        ...
        if (cachedData && cacheExpiry && new Date().getTime() < Number(cacheExpiry)) {
          const parsedData = JSON.parse(cachedData);
          setUserDetails(parsedData.filter(user => !deletedUserIDs.includes(user.id)));
      ...
      const filteredUsers = users.filter((user) => !deletedUserIDs.includes(user.id));
  ...
  useEffect(() => {
    const storedDeletedIDs = localStorage.getItem("deletedUser IDs");
    if (storedDeletedIDs) {
    ...
    >
       <div className="d-flex justify-content-between align-items-center p-3">
        <div>
          <p style={{
            fontSize:"25px"
          }} className="mb-0">Agent Listing</p>
